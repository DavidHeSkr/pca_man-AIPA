...
class new_state():
    ...
    def __init__(self, curr_posi, foods_posi, capsule_posi):
        self.curr_posi = curr_posi
        self.foods_posi = foods_posi
        self.capsule_posi = capsule_posi

    def get_curr(self):
        return self.curr_posi

    def get_foods(self):
        return self.foods_posi
...
class CapsuleSearchProblem(search.SearchProblem):
    ...
    def __init__(self, gameState, costFn = lambda x: 1, goal=(1,1), start=None, warn=True, visualize=True):
        ...
        #print("the capsule is :", self.capsule)
        """
        ...
        #print("the foods are: ", self.foods)
        if start != None: self.startState = start
        self.goal = goal
        self.costFn = costFn
        self.visualize = visualize
    ...
    def getStartState(self):
        returned_state = [self.startState, self.foods, self.capsule, []]
    ...
    def isGoalState(self, state):
        ...
        #print(state[1])
        #print(state[2])
        #print("state[2] is: _______________________________________________________,", self.gameState.getCapsules())
        if state[1].count() == 0 and len(state[2]) == 0:
        #if len(state[2]) == 0:
        #if len(state[1]) == 0:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if not capsule or not foods[nextx][nexty]:
                    # if next position is not a food, we take it
                        ...
                        nextState = [(nextx, nexty), newFood, newCapsule, state_[3]]
                        cost = self.costFn(nextState)
...
def foodHeuristic(state, problem):
    ...
    position, foodGrid, capsules, visited = state
    "*** YOUR CODE HERE ***"
    #print("*****************************************", foodGrid.count())
    return foodGrid.count() + len(capsules)
